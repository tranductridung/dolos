...
class Invisiport(Core):
    ...
    def __blacklist(self, ip, port):
        ...
        params = "iptables -A INPUT -s {} -p tcp ! --destination-port {} -j DROP".format(ip, Invisiport.PORT)
        subprocess.run(params,shell=True)
        super().log(Core.INFO, "Add rule to block IP {} in iptables".format(ip))
    
        params = "iptables -t nat -A PREROUTING -s {} -p tcp --dport {} -j REDIRECT --to-port {}".format(ip, port, Invisiport.PORT)
        subprocess.run(params, shell=True)
