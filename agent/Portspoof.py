...
class Portspoof(Core):
    class Signature:
        def __init__(self, filename, port, core_instance):
            self.__signature = ''
            raw_signatures = []
            try:
    ...
    #     super().log(Core.WARNING, "SOMEONE CONNECT TO PORTSPOOF2")
    #     del self._sig

    def __init__(self, sock, port, active_sock, ip):
        super().__init__(sock, port, active_sock, ip)
        self._sig = Portspoof.Signature('portspoof_signatures', port, self)
        self.answer = self._sig.get_signature()
        super().log(Core.WARNING, "Someone connect to Portspoof")
        del self._sig
